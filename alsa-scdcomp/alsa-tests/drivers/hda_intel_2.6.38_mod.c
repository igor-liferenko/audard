...
static int azx_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	snd_printdd(SFX "azx_pcm_prepare: bufsize=0x%x, format=0x%x, fifo_size=%d\n",
		    bufsize, format_val, azx_sd_readw(azx_dev, SD_FIFOSIZE));
...
static snd_pcm_uframes_t azx_pcm_pointer(struct snd_pcm_substream *substream)
{
  ...
  trace_printk("_pointer: %ld (%d) a:%lu h:%lu d:%ld av:%ld hav:%ld ss:%lu hb:%lu hi:%lu c:%pS\n", ploc, substream->stream, substream->runtime->control->appl_ptr, substream->runtime->status->hw_ptr, substream->runtime->delay, (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) ? snd_pcm_playback_avail(substream->runtime) : snd_pcm_capture_avail(substream->runtime), (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) ? snd_pcm_playback_hw_avail(substream->runtime) : snd_pcm_capture_hw_avail(substream->runtime), substream->runtime->silence_size, substream->runtime->hw_ptr_base, substream->runtime->hw_ptr_interrupt, __builtin_return_address(1));
  //~ dump_stack();
