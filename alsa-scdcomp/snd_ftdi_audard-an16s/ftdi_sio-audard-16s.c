...
*******************************************************************************/
/*
 * USB FTDI SIO driver - 'AudioArduino' modification (using snd_ftdi_audard_16s.h; 16s)
 * modification 2013 by sdaau (sd@{imi,create}.aau.dk)
 ...
 * Version Information
 */
#define DRIVER_VERSION "v1.5.0audard16s"
...
#define DRIVER_DESC "Audio Arduino 16s - USB FTDI Serial Converters Driver"

//"ftdi_sio"
// "ftdi_sio_audard" - 15 chars - OK
// "ftdi_sio_audardZ" - 16 chars - returns in aplay -l: 'ftdi_sio_audardZMyS'; same for "ftdi_sio_audardZZZZ"
// NOTE: in sound/core.h; snd_card struct defines: char id[16]; char driver[16]; char shortname[32]; char longname[80];
#define DRVNAME "ftdi_sio_auda16"
//"FTDI USB Serial Device"
#define SIODEVDESC "Audio Arduino 16s FTDI USB Serial Device"
...
#include "snd_ftdi_audard_16s_B.h" //currently only with ftdi overruns; _C may be worse
//~ #include "snd_ftdi_audard_16s_C.h"
//~ #include "ftdi_profiler.h"
...
static int ftdi_sio_port_probe(struct usb_serial_port *port)
{
	...
	if (port->bulk_in_buffer) kfree(port->bulk_in_buffer); // added; kedr leak check (doesn't do much)
	...
	if (port->bulk_out_buffer) kfree(port->bulk_out_buffer); // added; kedr leak check (doesn't do much)
...
static int ftdi_sio_port_remove(struct usb_serial_port *port)
{
  ...
  // kernel crashes with kfree(port->bulk_in_buffer); here
  // seem to be untreaceable on running system (no /var/crash; no /var/log/kern.log)
  //~ if (port->bulk_in_buffer) kfree(port->bulk_in_buffer); // added; kedr leak check // kernel crash here too
  ...
  //~ if (port->bulk_in_buffer) kfree(port->bulk_in_buffer); // added; kedr leak check // kernel crash here ?!
	remove_sysfs_attrs(port);

  //~ if (port->bulk_in_buffer) kfree(port->bulk_in_buffer); // added; kedr leak check  // kernel crash here too! (freeze + kernel panic)

	kref_put(&priv->kref, ftdi_sio_priv_release);

  //~ if (port->bulk_in_buffer) kfree(port->bulk_in_buffer); // added; kedr leak check  // kernel crash here too! (autoreboot)
		...
		char *packet, int len)
{
  ...
  //~ if (len > 2 && packet[1] != 0 && packet[1] != 0x60 && packet[1] != 0x62 ) // only 0x01 0x02 seems to match this
    //~ printk("ftdi: + len: %d; packet: 0x%02x 0x%02x '%s';\n", len, packet[0], packet[1], packet);
  if (len > 2 && len > 64)
    printk("ftdi: + len: %d; packet: 0x%02x 0x%02x;\n", len, packet[0], packet[1]);
	/*
	...
	if (packet[1] & FTDI_RS_OE) {
		flag = TTY_OVERRUN;
		printk("ftdi: OVERRRUN error  len: %d; packet: 0x%02x 0x%02x \n",  len, packet[0], packet[1]);
	}
	if (packet[1] & FTDI_RS_BI) {
		flag = TTY_BREAK;
		dbg("ftdi: BREAK received\n");
		usb_serial_handle_break(port);
	}
	if (packet[1] & FTDI_RS_PE) {
		flag = TTY_PARITY;
		dbg("ftdi: PARITY error\n");
	}
	if (packet[1] & FTDI_RS_FE) {
		flag = TTY_FRAME;
		dbg("ftdi: FRAMING error\n");
	}

  #ifdef __FTDI_PROFILER_H
  // send also status only
	if (priv->audev->running) {
		audard_xfer_buf(priv->audev, packet, len);
  }
  #endif
