...
# call with:
# python2.7 vcd2uint-gnuplot.py -i FILENAME.vcd 2>/dev/null > d_source.text
# python3.2 vcd2uint-gnuplot.py -i FILENAME.vcd 2>/dev/null > d_source.text
...
# dict for converting singular VCD values
...
def usage():
  ...
  logso('Usage:')
  logso('python2.7/3.2 ' + scriptcallpath + ' [-h] -i/--input-file=filename 2>/dev/null > d_source.text')
  logso('Stderr is used for messages (redirect to /dev/null to suppress)')
  logso('Stdout is used for converted file output (redirect to file to save)')
...
def generateOutput():
  ...
  # gnuplot uses # as comment character (ngspice uses * for d_source file)
  # otherwise, keep same format
  logso("# " + scriptcallpath + ": conversion of " + inputfile)
  logso("# uint format data (gnuplot)")
  logso("# time\t" +  "\t".join(tmpid1))
  logso("# time\t" +  "\t".join(tmpid2))
  ...
  for item in VCD_alldata:
    ...
    for ival in tvals:
      ...
      #~ if (len(ival[1]) == 1):
        #~ try:
          #~ curVals[ival[0]] = vcd2g_sing_d[ival[1]] # "Uz"
          #~ convok = 1
        #~ except:
          #~ convok = 0
      #~ if (convok == 0): # just set the value in this case - but ngspice will fail on it!
      #~ curVals[ival[0]] = ival[1]
      # note - ghdl outputs "b00100100";
      # python's int("0101",2) can parse either "0101" or "0b0101", but not "b0101" !!
      # so just prepend 0 to the string:
      curVals[ival[0]] = str(int("0" + ival[1],2))
