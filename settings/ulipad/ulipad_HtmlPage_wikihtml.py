...
class HtmlWindowBase:
    ...
    def DoRefresh(self, scrollbarxy=None):
        self.Freeze() # try first disable the redraw.. doesn't do much
        self.Load(self.filename)
        if scrollbarxy is not None:
            #~ print "inoxy2"
            (ox, oy) = scrollbarxy
            #~ print "OXY2 " + str(ox) + str(oy) + str(self)
            self.Scroll(ox, oy) # self.html
        self.Thaw()  # reenable redraw again
