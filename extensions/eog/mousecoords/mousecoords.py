...
class MouseCoordsPlugin(eog.Plugin):

  def __init__(self):
    eog.Plugin.__init__(self)
    self.console_window = None
    self.mousecoords = None
    self.handlerId1 = None
  ...
  def activate(self, window):
    ...
    data['group'] = gtk.ActionGroup('MouseCoords')
    data['group'].add_actions([('MouseCoords', None, 'Copy M_ouse Coords', "M", None, self.launchmenu_copyMouseCoords)], window)
    ui_manager.insert_action_group(data['group'], 0)
    data['ui_id'] = ui_manager.add_ui_from_string(ui_str)

    window.set_data('MouseCoordsPluginInfo', data)
    self.handlerId1 = window.connect('delete-event', self.self_deactivate)
    ...
    eog_context_id = window.get_statusbar().get_context_id("")
    #~ print "eog_context_id", eog_context_id
    self.handlerId3 = window.get_statusbar().connect('text-popped', self.status_text_popped)
  ...
  def deactivate(self, window):
    data = window.get_data('MouseCoordsPluginInfo')
    ui_manager = window.get_ui_manager()
    if(data):
      ui_manager.remove_ui(data['ui_id'])
      ui_manager.remove_action_group(data['group'])
      ui_manager.ensure_update()
    window.disconnect(self.handlerId1)
    window.disconnect(self.handlerId2)
    window.get_statusbar().disconnect(self.handlerId3)
    # window.get_image().disconnect(self.handlerId4)
    window.set_data("MouseCoordsPluginInfo", None)
    print 'MouseCoords plugin deactivated'

# /usr/lib/eog/plugins/pythonconsole.py:
...
# See bug #460781 for more information.
  ...
  def it_moved(self, window, event):
    ...
    if ( not(initdel) ):
      ...
      window.get_statusbar().pop(eog_context_id) # returns string, doesn't delete
    else:
      window.get_statusbar().pop(2) # our own we have to delete with a different context it (sort of?)

    #~ print r[1], r[2]
    # should do window.get_statusbar().get_context_id(""); but returns wrong
    ...
    #window.get_statusbar().pop(eog_context_id)
    return True
  ...
  def status_text_popped(self, statusbar, context_id, text):
    ...
    # from python console:
    # window.get_statusbar().pop(3) -> here lists c 1, gives text, no delete
    # window.get_statusbar().pop(1) -> here lists c 0, no text, deletes
    ...
    if (context_id == 0):
      ...
      if (self.mousecoords is not None):
        ...
        self.laststatustext = lastsbartext + \
          " si:[%d,%d]" % (self.imscw, self.imsch) + \
          " w:["+ str(self.mousecoords[0]) +","+ str(self.mousecoords[1]) +"]" + \
          " i:[%d,%d]" % (self.mouse_imgcoord_x, self.mouse_imgcoord_y) + \
          " vp:" + self.get_scviewport_IMstring()
  ...
  def compute_scaled_size(self, window):
    ...
    self.imscw = math.floor(self.imwidth*self.zoom + 0.5);
    self.imsch = math.floor(self.imheight*self.zoom + 0.5);
  ...
  def compute_zoom_offsets(self):
    ...
    hajd = self.hslide.get_adjustment()
    vajd = self.vslide.get_adjustment()
    ...
    #~ print("H",
      ...
      #~ " psz ", hajd.page_size,
      #~ " upp ", hajd.upper,
      #~ " s_i ", hajd.step_increment,
      #~ " p_i ", hajd.page_increment
      #~ )
    #~ print("V",
      #~ " low ", vajd.lower,
      #~ " val ", vajd.value,
      #~ " psz ", vajd.page_size,
      #~ " upp ", vajd.upper,
      #~ " s_i ", vajd.step_increment,
      #~ " p_i ", vajd.page_increment
      #~ )
    ...
    if (self.imscw <= self.scrollviewalloc.width):
      ...
      # so set it to -1
      if (self.mousecoords[0] < offsx) or (self.mousecoords[0] > self.scrollviewalloc.width-offsx):
        self.mouse_imgcoord_x = -1
      else:
      ...
      self.scrollfact_offx = hajd.value/(hajd.upper-hajd.lower)
      self.viewport_offset_imgx = self.scrollfact_offx*self.imwidth
      self.scrollfact_szx = hajd.page_size/(hajd.upper-hajd.lower)
    ...
    if (self.imsch <= self.scrollviewalloc.height):
      ...
      self.scrollfact_offy = vajd.value/(vajd.upper-vajd.lower)
      self.viewport_offset_imgy = self.scrollfact_offy*self.imheight
      self.scrollfact_szy = vajd.page_size/(vajd.upper-vajd.lower)
  ...
  # copies statusbar text (modified by this plugin) to clipboard
  def launchmenu_copyMouseCoords(self,action, window):
    print "launchmenu_copyMouseCoords " + self.laststatustext
    timstr = self.get_test_IMstring()
    print timstr
    self.clipboard = gtk.clipboard_get(gtk.gdk.SELECTION_CLIPBOARD)
    # just test - get clipboard text (with callback):
    # self.clipboard.request_text(self.clipboard_text_received)
    # set clipboard contents
    self.clipboard.set_text(self.laststatustext)

    # important to check legal geometry before executing,
    # else `display` will cause a freeze!
    if ((self.imscw >= self.scrollviewalloc.width) and (self.imsch >= self.scrollviewalloc.height)):
      # # os.system(timstr) # freezes eog until `display` exit
      # subprocess.Popen doesn't freeze eog until `display` exit;
      # but when eog exits, also nice that `display` windows are killed
      vpstr = self.get_scviewport_IMstring()
      fnstr = self.image.get_file().get_path()
      subprocess.Popen(["display","-crop",vpstr,fnstr], stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)


  # just for test - OK
  # http://pygtk.org/pygtk2tutorial/examples/clipboard.py
