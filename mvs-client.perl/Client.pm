...
require WWW::Mediawiki::Client::Exceptions;
use XML::LibXML ();
...
use constant SAVED_ATTRIBUTES => (
    qw(site_url host protocol language_code space_substitute username
       password wiki_path watch encoding minor_edit escape_filenames ausername apassword arealm)
...
=head2 ausername
  ...
  my $url = $mvs->ausername($url);

The ausername for basic HTTP authentication - separate from MediaWiki user credentials.

=cut

sub ausername {
    my ($self, $ausername) = @_;
    $self->{ausername} = $ausername if $ausername;
...
=head2 apassword
  ...
  my $url = $mvs->apassword($url);

The apassword to use for basic HTTP authentication.  Note that maybe this apassword is sent I<en clair>, so it's probably not a
good idea to use an important one.
...
sub apassword {
    my ($self, $apassword) = @_;
    $self->{apassword} = $apassword if $apassword;
    return $self->{apassword};
}

=head2 arealm

  my $url = $mvs->arealm($url);

The arealm to use for basic HTTP authentication.  

=cut

sub arealm {
    my ($self, $arealm) = @_;
    $self->{arealm} = $arealm if $arealm;
    return $self->{arealm};
}
...
sub _update_core {
	...
	#print STDERR "\r\npagename $pagename dirs $dirs filename $filename \r\n";
    my $sv = $self->get_server_page($pagename);
	#print STDERR "\r\n after get_server_page \r\n";
...
=head2 do_commitnr
  ...
  $self->do_commitnr($filename);

The C<do_commitnr> - commit no revision - method should be 
able to update only  the text (old_text) in a MediWiki db 
(text table), without taking  a revision.. 
...
do_commitnr {
    my ($self, $filename) = @_;
    #WWW::Mediawiki::Client::CommitMessageException->throw(
    #        "No commitnr message specified")
    #    unless $self->{commitnr_message};
	#print STDERR "do_commitnr: filename $filename ... ";
    # Perform the actual upload:
	# set the $commit parameter to 2
    my ($res, $text) = $self->_upload_file($filename, 2);
    # print $text;
	#print STDERR "do_commitnr: after _upload_file ... ";
    # save the local version as the reference version
    my $refname = $self->_get_ref_filename($filename);
	#print STDERR "do_commitnr: after _get_ref_filename ... \r\n";
    open OUT, ">:utf8", $refname 
            or WWW::Mediawiki::Client::FileAccessException->throw(
            "Cannot open $refname for writing.");
    print OUT $text;
...
This is an alias for C<do_commitnr>.
...
sub load_state {
    my $self = shift;
	BEGIN { push @INC,'.'; } # fix for pp- compiled on Windows...
	#print STDERR "1 INC @INC";
...
sub get_server_page {
	...
	#print STDERR "\r\n get_server_page: before my $export = delete \r\n";
    return $export if defined($export);
	
	#print STDERR "\r\n get_server_page: after my $export = delete \r\n";

    my $url = $self->pagename_to_url($pagename, EDIT);
	#print STDERR "\r\n get_server_page: my url $url \r\n";
    my $res = $self->{ua}->get($url);
	#print STDERR "\r\n get_server_page: my res $res ".$res->is_success . "\r\n";
	...
	# print STDERR "\r\n get_server_page: my doc $doc \r\n";
    # don't encode here (same for decode) - then bogus chars are added to unicodes
    # my $text = Encode::encode("utf8", $self->_get_wiki_text($doc));
    my $text = $self->_get_wiki_text($doc);
    # print $text . "\r\n000 get_server_page \r\n";
    $self->{edit}->{date} = $self->_get_edit_date($doc);
	#print STDERR "\r\n get_server_page: _get_wiki_text(doc) $text self->{edit}->{date} ".$self->{edit}->{date}."\r\n";
	...
	#print STDERR "\r\n get_server_page: expected $expected headline $headline\r\n";
    ...
    chomp $text;
	#print STDERR "\r\n get_server_page: after chomp, pre return, text =.$text.= \r\n";
    return $text;
}
...
sub pagename_to_url {
    ...
    my $protocol = $self->protocol;
	#print STDERR "pagename_to_url $name \r\n";
...
sub filename_to_pagename {
    ...
    $name =~ s/_/ /g;
	#print STDERR "filename_to_pagename $name \r\n";
    return ucfirst $name;
}
...
sub _get_page_headline {
    ...
    my $p = HTML::TokeParser->new(\$doc);
    my $kx = $p->get_tag("h1");
	# h1 attr HASH(0x21efbd4) attrseq ARRAY(0x21f01cc) rawtxt <h1 id="logo">
	# my($tag, $attr, $attrseq, $rawtxt) = @{ $kx };
	# can be called twice!? - second time $kx2 being undefined?
    my $text = $p->get_text; #It will return a zero length string if the next token is not text. 
	if($text eq "") {
		my $kx2 = $p->get_tag("h2"); #print STDERR "\r\n aa ".($kx2 eq "")."\r\n";
		if (defined $kx2) { 
			$text = $p->get_text; 
			my($tag, $attr, $attrseq, $rawtxt) = @{ $kx2 };
		}
	}
	#print STDERR "\r\n _get_page_headline: text $text kx tag $tag attr $attr attrseq $attrseq rawtxt $rawtxt\r\n";
    $text =~ s///gs;                      # convert endlines
    return $text;
}
...
sub _upload_file {
    ...
    if ($self->{edit}->{date}) {
        ...
        WWW::Mediawiki::Client::UpdateNeededException->throw(
                error => $self->filename_to_pagename($filename) 
                       . " has changed on the server." . $self->{edit}->{date},
            ) unless $sp eq $ref;
    }
    chomp ($text);
    ...
    my $url;
    if ($commit == 2) {
		$url = $self->filename_to_url($filename, 'commitnr');
	} else {
		$url = $self->filename_to_url($filename, SUBMIT); }
    # some systems like this, some not (for unicode chars within .wiki)
    # my $octets = Encode::encode($self->encoding, $text);
    my $octets = $text;
	# print STDERR "_upload_file $url url act_value $act_value commit $commit \n"; 
	# print  "     " . &TEXTAREA_NAME . " octets $octets \n"; 
	# print STDERR "     " . &COMMENT_NAME . " self->{commit_message} ".$self->{commit_message}." \n"; 
	# print STDERR "     " . ($commit == 2) . " or ".($commit == "2")." \n"; 
    ...
    unless (lc($headline) eq lc($expect)) {
        ...
        WWW::Mediawiki::Client::CommitException->throw(
	        error => "When we tried to commit '$pagename' the server responded with '$headline'. doc: '$doc'",
                res   => $res,
            ) if ($headline);
    }
    return ($res, $text);
