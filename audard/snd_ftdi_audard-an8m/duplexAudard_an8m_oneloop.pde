...
Non-interrupt (one-loop) version 
...
// period corresponding to ARate for F_CPU (=16MHz),
...
static const unsigned int periodARateCmp = periodARate-26;
...
{ 
  ...
  //~ sei(); // Enable interrupts once registers have been updated - no need here?
...
void loop() 
{
  ...
  // get timestamp - actually, use TCNT1 directly, so no need
  //~ timerCountTS_now = TCNT1; 
  ...
  *outl |= bitl; //HIGH (LED Dout13)
    ...
    OCR0A = rx_buffer.data[rx_buffer.tail]; //.pos];
  ...
  *outl &= ~bitl; // LOW (LED Dout13)
  ...
  // check nonetheless RX - drain/flush and put in buffer, if any
  
  // check - has enough time passed, to match period of f=44100? 
  // if so - write and read a single sample - not anymore: 
  
  // now, we do that (filling buffer with incoming USB) 
  // while we wait with `while` (that the TCNT1 elapses periodARate):
  while (TCNT1 < periodARateCmp) {
    ...
    rx_buffer.pos += _c; //(_c >> RXC0);
    rx_buffer.data[rx_buffer.pos] = UDR0;
  }
  
  
  // as timestamp - get 16-bit timer1 value directly
  //~ _c = (timerCountTS_now - timerCountTS >= periodARate);
  //~ if (timerCountTS_now - timerCountTS >= periodARate) {
  // NEW strategy - don't `if` here; wait with `while` at end instead
    
      
    // reset timestamp - not anymore:
    //~ timerCountTS = TCNT1;
  //~ }

  // now reset TCNT1 
  //   it should also count while loop `ret` executes, which should be tighter timing
  TCNT1 = 0; 
